...
class Pair(Dataset):
    ...
    def __init__(self, seqs, transforms=None,
        ...
        # print(self.cfg_anchor)
        self.anchors = ops.create_anchors(self.cfg_anchor , int((self.cfg_anchor.instance_sz - self.cfg_anchor.exemplar_sz) / self.cfg_anchor.total_stride + 1))
    ...
    def parse_args(self, **kargs):
        ...
        for key, val in kargs.items():
            self.cfg_anchor.update({key: val})
        self.cfg_anchor = namedtuple('GenericDict', self.cfg_anchor.keys())(**self.cfg_anchor) 
    ...
    def __getitem__(self, index):
        ...
        box_z = anno[rand_z]
        box_x = anno[rand_x]
        # ops.draw_rect(z , box_z , 'z.jpg')
        ops.draw_rect(x , box_x , 'x.jpg')     
        # toten = ToTensor()
        # z_new_pic = ops.Tensor_to_CV(toten(z))
        # ops.draw_rect(z_new_pic , name = 'z2.jpg')

        if self.transforms is not None:
            z_new , x_new , gt_size = self.transforms(z, x, box_z, box_x)
        # print(z_new.shape)
        # print(x_new.shape)        
        # x_new_pic = ops.Tensor_to_CV(x_new)    
        # print(x_new_pic.shape)     
        # ops.draw_rect(x_new_pic ,  name = 'x_new.jpg')  
        # ops.draw_rect(x_new_pic , boxes = gt_size , name = 'x_new_with_gt.jpg' , box_fmt = 'centerhw')    

        regression_target, conf_target = ops.compute_target(self.anchors,np.round(gt_size)) 


        conf_target = from_numpy(conf_target)
        regression_target = from_numpy(regression_target).float()
        item_new = (z_new , x_new ) 
        # print(z_new.shape)
        # print(x_new.shape)
        # print(regression_target.shape)
        # print(conf_target.shape)
        return item_new , conf_target , regression_target
        # return item_new , conf_target , regression_target 
